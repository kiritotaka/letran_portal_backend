...
OOXML_PARTS = {
    ...
    ".pptx": ("ppt/presentation.xml", "{http://schemas.openxmlformats.org/presentationml/2006/main}presentation",
...
def inspect_file(name, content):
    ...
    if ext not in MIMES:
        raise ApiError(415, "UNSUPPORTED_FILE_TYPE", "Use JPG, PNG, PDF, DOCX, XLSX, PPTX or TXT.")
    try:
...
def sign(client, data):
    try:
        result = client.storage.from_(data["bucket"]).create_signed_url(
            data["object_path"], SIGNED_URL_TTL, {"download": data["original_name"]})
        url = result["signedURL"]
        if not isinstance(url, str) or not url.startswith("https://"):
            raise ValueError()
    except Exception:
        raise ApiError(503, "STORAGE_UNAVAILABLE", "Download URL is unavailable.") from None
    return {**data, "download_url": url, "expires_in": SIGNED_URL_TTL}


def upload_file(client, actor, request_id, file_id, upload):
    content = upload.file.read(MAX_FILE_BYTES + 1)
    name, mime = inspect_file(upload.filename, content)
    repo = ServiceFileRepository(client)
    data = repo.write(actor, "reserve_file", request_id=str(request_id), file_id=str(file_id),
                      original_name=name, content_type=mime, size_bytes=len(content),
                      sha256=hashlib.sha256(content).hexdigest())
    if data["status"] == "ready":
        return sign(client, data)
    try:
    ...
    try:
        data = repo.write(actor, "complete_file", request_id=str(request_id), file_id=str(file_id))
...
def download_url(client, request_id, file_id):
    repo = ServiceFileRepository(client)
    rows = repo.one("portal_service_files", file_id)
    if str(rows["request_id"]) != str(request_id):
        raise ApiError(404, "FILE_NOT_FOUND", "File is not in this request.")
    if rows["status"] != "ready":
        raise ApiError(409, "FILE_NOT_READY", "File is not available for download.")
    signed = sign(client, rows)
    return {"file_id": str(file_id), "download_url": signed["download_url"], "expires_in": SIGNED_URL_TTL}
